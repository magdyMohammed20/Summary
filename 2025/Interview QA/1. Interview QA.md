...
==> Psueudo Class : target states like ': hover' or ': active', while pseudo-elements like ' ::before ' or:: after' style specific parts of an element
...
(53) reconciliation
----------------------
==> is the process by which React updates the DOM to match the virtual DOM efficiently. It involves comparing the new virtual DOM tree with the previous one and determining the minimum number of changes required to update the actual DOM. This process ensures optimal performance by avoiding unnecessary re-renders

(54) React portals
-------------------
==> allow rendering children into a DOM node outside the parent component's hierarchy. This is useful for modals or tooltips that need to escape parent overflow or z-index constraints.

(55) lazy loading
-------------------
==> technique where components are loaded only when they are needed, rather than at the initial page load. This helps reduce the initial load time and improve performance by splitting the code into smaller chunks.


(56) Timer Count Up With [start - stop - reset]
-------------------------------------------------
...
const Timer = () => {
  ...
  const start = () => {
    intervalRef.current = setInterval(() => {
      ...
      timeRef.current.innerText = formatTime(time.current);
    }, 1);
  };
  ...
  return (
    <div className="timer-container">
      <h1 className="timer-text" ref={timeRef}></h1>
...
(1) what is diff between react and vue and when decide to work with react/vue

(2) what occurs after user write url in the browser behind the scenes in react and vue
